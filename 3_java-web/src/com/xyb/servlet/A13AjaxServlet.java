 ...
 * 服务器端Json的使用，需要gson包
 */
public class A13AjaxServlet extends BaseServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if ("jqPostAjax".equals(req.getParameter("action"))) {
            System.out.println("成功调用到jqPostAjax A13AjaxServlet，返回字符串为：" + jsonStr);
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if ("jqGetAjax".equals(req.getParameter("action"))) {
            System.out.println("成功调用到jqGetAjax A13AjaxServlet，返回字符串为：" + jsonStr);
            this.out.write(jsonStr);
        }


        if ("jqJsonGetAjax".equals(req.getParameter("action"))) {
            System.out.println("成功调用到jqJsonGetAjax A13AjaxServlet，返回字符串为：" + jsonStr);
            this.out.write(jsonStr);
        }

        if ("serializeAjax".equals(req.getParameter("action"))) {
            System.out.println("成功调用到serializeAjax A13AjaxServlet，返回字符串为：" + jsonStr);
            this.out.write(jsonStr);
        }
